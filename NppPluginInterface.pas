...
type
    ...
    { Extended with utility functions. }
	TNppData = record
		nppHandle: HWND;
		scintillaMainHandle: HWND;
		scintillaSecondHandle: HWND;
    private
    	function GetString(aMsg: cardinal): string;
    public
        ...
        function GetExtPart: string;
        function GetCurrentWord: string;
        function GetNppDirectory: string;
	end;
...
function TNppData.GetString(aMsg: cardinal): string;
var
    ret: integer;
    buf: array[0..MAX_PATH] of char;
begin
    ret := SendMessage(nppHandle, aMsg, MAX_PATH, WPARAM(@buf[0]));
    if (ret <= 0) then
        Exit;
    Result := string(buf);
end;

function TNppData.GetOpenFiles: TArray<string>;
var
    ret: integer;
    ary: array of pchar;
    i: integer;
begin
    ret := SendMessage(nppHandle, NPPM_GETNBOPENFILES, 0, ALL_OPEN_FILES);
    if (ret <= 0) then
        Exit;
    SetLength(ary, ret);
    try
        ...
        ret := SendMessage(nppHandle, NPPM_GETOPENFILENAMES, WPARAM(@ary[0]), ret);
        if (ret <= 0) then
            Exit;
...
function TNppData.GetFullCurrentPath: string;
begin
	Result := GetString(NPPM_GETFULLCURRENTPATH);
end;

function TNppData.GetCurrentDirectory: string;
begin
	Result := GetString(NPPM_GETCURRENTDIRECTORY);
end;

function TNppData.GetFilename: string;
begin
	Result := GetString(NPPM_GETFILENAME);
end;

function TNppData.GetNamePart: string;
begin
	Result := GetString(NPPM_GETNAMEPART);
end;

function TNppData.GetExtPart: string;
begin
	Result := GetString(NPPM_GETEXTPART);
end;

function TNppData.GetCurrentWord: string;
begin
	Result := GetString(NPPM_GETCURRENTWORD);
end;

function TNppData.GetNppDirectory: string;
begin
	Result := GetString(NPPM_GETNPPDIRECTORY);
end;
